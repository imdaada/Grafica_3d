	...
	ray(x1, y1, z1, x2, y2, z2, lights) {
		for (let plan of this.cords_planes) {
			...
			for (let plan1 of this.cords_planes) {
				...
				/*if(a*(x2-x1) + b*(y2-y1) + c*(z2-z1) + d < 0) {
					flag = false;
					break;
				}*/
			}
			if (flag) {
				...
				/(Math.sqrt(a*a + b*b+c*c)*Math.sqrt((light.vector[0]*light.vector[0])+(light.vector[1]*light.vector[1])+(light.vector[2]*light.vector[2]))); //между нормалью и углом падения
...
class Reytreiser {
	...
	ray(x, y, z, Aangle, Bangle) {
		...
		for (let fig of this.objects) {
			 let col1 = fig.ray(xk1, yk1, zk1, 
			 xp1, yp1, zp1, this.lights);
			 colorPix[0] += col1[0];
			 colorPix[1] += col1[1];
			 colorPix[2] += col1[2];
		}
	...
	reytraise(Aangle, Bangle) {
		...
		for (let x = -this.screenSize[0]/2; x < this.screenSize[0]/2; x++) {
			for (let y = -this.screenSize[1]/2; y < this.screenSize[1]/2; y++) {
				this.ray(x , y, this.camPosition[2]+100, Aangle, Bangle);
			}
		}
		this.colorPixelsF();
	}
...
let ret = new Reytreiser([500, 500], [0, 0, -350], [new Kub([0, 0, 0], [0, 0, 0, 0])], [lig1, lig2, lig3]);
